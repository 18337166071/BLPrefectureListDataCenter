...
+ (NSDate *)bl_dateWithString:(NSString *)dateString{
...
+ (NSDate *)bl_dateWithString:(NSString *)dateString dateFormatter:(NSString *)dateFormatter
{
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
...
- (NSString *)dateTimeStringWithDateFormatter:(NSString *)dateFormatter
{
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    formatter.dateFormat = dateFormatter;
...
- (NSString *)dateStringLater:(NSInteger)days {
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    formatter.dateFormat = D_BII_DATE_Format;
    return [formatter stringFromDate:[self dateByAddingDays:days]];
}
...
- (NSDate *) dateByAddingHours: (float) dHours
{
...
- (NSString *)distanceNowDate:(NSTimeInterval)timeInterval
{
    NSString *string = @"";
    ...
    if (timeInterval) {
        NSDate *beginDate = [NSDate dateWithTimeIntervalSince1970:timeInterval/1000.0];
        NSTimeInterval betweenTWoTimes = [[NSDate date] timeIntervalSinceDate:beginDate];
        if (betweenTWoTimes < 60) {
            string = [NSString stringWithFormat:@"刚刚"];
        }else if(60 <= betweenTWoTimes && betweenTWoTimes < 3600){
            string = [NSString stringWithFormat:@"%d分钟前",(int)betweenTWoTimes/60];
            NSLog(@"%d分钟前",(int)betweenTWoTimes/60);
        }else if(3600 <= betweenTWoTimes && betweenTWoTimes < 3600*24){
            string = [NSString stringWithFormat:@"%d小时前",(int)betweenTWoTimes/60/60];
            NSLog(@"%d小时前",(int)betweenTWoTimes/60/60);
        }else if(3600*24 <= betweenTWoTimes && betweenTWoTimes < 3600*24*30){
            string = [NSString stringWithFormat:@"%d天前",(int)betweenTWoTimes/60/60/24];
            NSLog(@"%d天前",(int)betweenTWoTimes/60/60/24);
        }else if (3600*24*30 <= betweenTWoTimes && betweenTWoTimes < 3600*24*30*12){
            string = [formatter2 stringFromDate:beginDate];
        }else{
            string = [formatter1 stringFromDate:beginDate];
        }
    }
    return string;
}
